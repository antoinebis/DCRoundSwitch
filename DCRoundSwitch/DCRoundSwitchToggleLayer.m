...
- (void)drawInContext:(CGContextRef)context
{
    ...
    if (self.offImage) {
        CGPoint offImagePoint = CGPointMake(textSpaceWidth + (textSpaceWidth - self.onImage.size.width) / 2.0 + knobRadius * .86, floorf((self.bounds.size.height - self.onImage.size.height) / 2.0) - 2.0);
